...
-- | Module-specific information for analysis
data AnalysisInfo = AnalysisInfo
  { weederConfig :: Config
  , currentHieFile :: HieFile
      -- ^ The current HieFile being analysed
  , refMap :: RefMap TypeIndex
  }


newtype AnalysisLog = AnalysisLog { unLog :: Seq String }
  deriving 
    ( Semigroup, Monoid )


tellS :: MonadWriter AnalysisLog m => String -> m ()
tellS = tell . AnalysisLog . pure
...
analyseHieFile :: (MonadReader AnalysisInfo m, MonadWriter AnalysisLog m, MonadState Analysis m) => m ()
analyseHieFile = do
  HieFile{hie_module, hie_hs_file, hie_asts = HieASTs hieASTs, hie_exports} <- asks currentHieFile
  tellS $ "Analysing " <> (show . moduleName) hie_module <> " from " <> hie_hs_file
...
analyseHieFiles :: (Foldable f, MonadRWS Config AnalysisLog Analysis m) => f HieFile -> m ()
analyseHieFiles hieFiles = do
  flags <- ask

  for_ hieFiles \hf ->
    let info = AnalysisInfo flags hf rf
...
addInstanceRoot x t cls = do
  ...
  -- NOTE: we could generalise @TypeIndex@ to @a@ and @Analysis@ 
  -- to @Analysis a@ and do the step of constructing the string in @Main.hs@
  str <- lookupPprType t 
...
topLevelAnalysis :: (MonadReader AnalysisInfo m, MonadState Analysis m) => HieAST TypeIndex -> m ()
...
-- | Find all evidence uses under this node and connect the declaration to
-- their instance binding sites.
connectEvidenceUses :: (MonadReader AnalysisInfo m, MonadState Analysis m) => HieAST a -> Declaration -> m ()
connectEvidenceUses n d = do
  noEvidence <- asks (typeClassRoots . weederConfig)
  rf <- asks refMap

  let getEvidenceTrees = mapMaybe (getEvidenceTree rf)
  ...
  -- if type-class-roots flag is set then we don't need to follow evidence uses as the
  -- binding sites will be roots anyway
  unless noEvidence $ for_ instanceEvidenceInfos \ev -> do

...
connectToUsedTypes n d = do
  ...
  for_ (map astTypes $ flattenAst n) \types ->
    let names = Set.unions $ map (typeToNames . (`recoverFullType` hie_types)) types
        ds = Set.map nameToDeclaration names
     in traverse_ (traverse_ (addDependency d)) ds
